...
class KakaocertController extends Controller
{
    ...
    public function RequestVerifyAuth(Request $request) {
        ...
        try {
            ...
            return response()->json([
                "code" => 2, "message" => $message, "data" => $code
            ]);
        }

        return response()->json([
            "code" => 1, "message" => "success!", "data" => [ "receiptID" => $receiptID ]
        ]);
    }
  ...
  */
  public function GetVerifyAuthState(Request $request){
    ...
    try {
      ...
      return response()->json([
          "code" => 1, "message" => "success!", "data" => [ "result" => $result ]
      ]);
  ...
  */
  ...
  public function verifyAuthRequest($receiptID) {
      ...
      try {
          ...
          $code = $ke->getCode();
          $message = $ke->getMessage();
          return [ "code" => 2, "message" => $message, "data" => $code ];
      }
  ...
  * 전자서명 서명을 요청합니다.
  */
  public function RequestESign(Request $request) {
    ...
    // 수신자 생년월일, 형식 : YYYYMMDD
  	$RequestESign->ReceiverBirthDay = $birth;

    // 수신자 휴대폰번호
  	$RequestESign->ReceiverHP = $hp;

    // 수신자 성명
  	$RequestESign->ReceiverName = $name;
  ...
  * 전자서명 서명 상태를 확인합니다.
  */
  public function GetESignState(Request $request) {
  ...
  * - 카카오페이 서비스 운영정책에 따라 검증 API는 1회만 호출할 수 있습니다. 재시도시 오류처리됩니다.
  */
  public function VerifyESign(Request $request){
