...
use App\Uploads\PeriodicalsAttachment;
use Illuminate\Support\Facades\Request;

class Periodicals extends Boards
{
    // 테이블명, 필수
    protected string $table = "board_periodicals";

    // 프론트->백으로 넘어올 input name값. 반드시 디비의 컬럼명과 동일해야함, 필수
    // 여기에 정의 안되면 디비에 안들어감
    protected array $columns = [ "title", "attachment1", "thumbnail", "from_date" ];
    ...
    {
        $this->attachmentTable = new PeriodicalsAttachment();
        $this->insert($request);
        return $this->getId();
    }
