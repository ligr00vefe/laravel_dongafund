...
class CreateDonationsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('donations', function (Blueprint $table) {
            ...
            $table->string("host")->nullable()->comment("기부권유자");
            $table->string("contract_code")->comment("약정서번호");
            $table->string("merchant_uid")->nullable()->comment("아임포트결제할때 상품id");
            ...
            $table->tinyInteger("receipt_check")->comment("기부자 영수증 발급용도(필수수집정보)")->nullable();
            $table->tinyInteger("benefit_check")->comment("기부자 예우혜택 제공 용도(선택수집정보)")->nullable();
            $table->tinyInteger("tax_check")->comment("세제혜택 편의 제공, 기부금 결제(제3자 제공정보)")->nullable();
            ...
            $table->dateTime("signature_datetime")->comment("전자서명성공일시")->nullable();

            $table->string("receiptId", "50")->comment("카카오전자서명 성공하면 저장해야하는 아이디")->nullable();
            $table->text("signedData")->comment("카카오전자서명 성공하면 저장해야하는 sign값")->nullable();
            ...
            $table->string("ip", "100")->comment("ip")->nullable();
