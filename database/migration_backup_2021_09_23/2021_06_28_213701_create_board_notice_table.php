...
class CreateBoardNoticeTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('board_notice', function (Blueprint $table) {
            ...
            $table->string("category")->default("기부소식")->comment("분류");
            ...
            $table->integer("thumbnail")->comment("썸네임")->nullable();
            $table->dateTime("from_date")->comment("시작일자")->nullable();
            $table->dateTime("to_date")->comment("종료일자")->nullable();
            $table->string("space1")->comment("취재")->nullable();
            $table->string("space2")->comment("사진")->nullable();
            $table->string("space3")->comment("여유공간3")->nullable();
