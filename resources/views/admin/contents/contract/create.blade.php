...
@section("title")
    컨텐츠 관리 <i class="fa fa-chevron-right" style="margin:0 8px;"></i> 협약서 <i class="fa fa-chevron-right" style="margin:0 8px;"></i> 신규등록/수정
@endsection
...
@section("content")
    ...
    <form action="{{ $action }}" onsubmit="return registerFormCheck(this)" method="post" enctype="multipart/form-data">
        ...
        <div class="contents-body">
                ...
                <table>
                    <tbody>
                        <tr>
                            <th>체결번호</th>
                            <td class="">
                                <input type="text" name="space1" value="{{ $contract->space1 ?? "" }}" placeholder="2021-0001">
                            </td>
                        </tr>
                        <tr>
                            <th>제목</th>
                            <td class="">
                                <input type="text" name="title" value="{{ $contract->title ?? "" }}" placeholder="동아100년동행">
                            </td>
                        </tr>
                        <tr>
                            <th>상대기관</th>
                            <td class="">
                                <input type="text" name="space2" value="{{ $contract->space2 ?? "" }}" placeholder="부산테크노파크">
                            </td>
                        </tr>
                        <tr>
                            <th>주관부서</th>
                            <td class="">
                                <input type="text" name="space3" value="{{ $contract->space3 ?? "" }}" placeholder="국제교류과">
                            </td>
                        </tr>
                        <tr>
                            <th>체결일자</th>
                            <td class="">
                                <input type="text" name="from_date" class="from_date" value="{{ $from_date ?? "" }}" placeholder="yyyy-mm-dd">
                            </td>
                        </tr>
                        <tr>
                            <th>첨부파일 업로드</th>
            ...
            <div class="btn-wrap btn-create">
                <a href="/b1BjW55p/contents/contract" class="btn-cancel"><i class="fas fa-undo"></i>취소</a>
    ...
    <script>
        function registerFormCheck(form){
            ...
            var rawDate = $('.from_date').val();
            form.from_date.value = rawDate.substr(0, 10);
